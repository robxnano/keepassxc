...
int DatabaseInfo::executeWithDatabase(QSharedPointer<Database> database, QSharedPointer<QCommandLineParser>)
{
    ...
    out << QObject::tr("Database created") << ": "
        << locale.toString(database->rootGroup()->timeInfo().creationTime(), QLocale::ShortFormat) << Qt::endl;
    out << QObject::tr("Last saved") << ": " << locale.toString(stats.modified, QLocale::ShortFormat) << Qt::endl;
